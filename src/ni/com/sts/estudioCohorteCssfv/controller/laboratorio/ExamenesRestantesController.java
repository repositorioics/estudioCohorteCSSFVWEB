...
public class ExamenesRestantesController extends SelectorComposer<Component> {
	...
	private int SEC_EXAMEN = 0;
	
	@Wire("[id$=resultadoExamenMuestra]")
	private Window wResultadoExamen;
	...
	@Wire("[id$=lblExamenRestante]")
	private Label lblExamenRestante;
	...
	@Listen("onCreate=[id$=resultadoExamenMuestra]")
	public void onCreate_resultadoExamenMuestra() throws Exception{
	  init();
	...
	private void init() throws Exception {
		...
		ordenesExamenes = (OrdenesExamenes) wResultadoExamen
				.getAttribute("ordenesExamenes");
		this.lblExamenRestante.setValue(ordenesExamenes.getExamen());
		ResultadoExamenMuestra resultado = laboratorioService.obtenerResultadoExamen(ordenesExamenes.getSecOrdenLaboratorio());
		if(resultado != null && resultado.getSecResultadoExamen() != 0){
			//txtCodigoMuestra.setText(resultado.getCodigoMuestra());
			SEC_EXAMEN = resultado.getSecResultadoExamen();
 ...
 @Listen("onClick=[id$=btnCerrar]")
    public void btnCerrar_onClick() {
	 wResultadoExamen.setAttribute("accionGuardado", false);
	 wResultadoExamen.onClose();
    }
...
private void guardarResultadoExamen() throws Exception{
	 
	ResultadoExamenMuestra resultadoExamen = new ResultadoExamenMuestra();;
	...
	//SimpleDateFormat sdfHoraReporte = new SimpleDateFormat("hh:mm a");
		...
		if(SEC_EXAMEN != 0){
			resultadoExamen.setSecResultadoExamen(SEC_EXAMEN);
		}
	 
	 resultadoExamen.setFechaHoraReporte(UtilDate.DateToString(horaReporte.getTime(), "dd/MM/yyyy HH:mm:ss"));
	 ...
	 resultadoExamen.setOrdenLaboratorio(ordenLaboratorio);
	 //resultadoExamen.setCodigoMuestra(this.txtCodigoMuestra.getValue());
	 resultadoExamen.setUsuarioBioanalista(bioanalista);
	 resultadoExamen.setEstado('1');

	 InfoResultado result = laboratorioService.guardarResultadoExamen(resultadoExamen);
	 if(result.isOk() && result.getObjeto()!=null){
		 resultadoExamen = (ResultadoExamenMuestra)result.getObjeto();
		 
		 Mensajes.enviarMensaje(Mensajes.REGISTRO_GUARDADO, 
				 Mensajes.TipoMensaje.INFO);
		 wResultadoExamen.setAttribute("accionGuardado", true);
		 wResultadoExamen.onClose();
...
public void chkTomaMx_onCheck() {
	...
	wResultadoExamen.setAttribute("accionTomaMx", true);
}
