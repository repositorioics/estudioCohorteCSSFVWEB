...
public class MuestraInfuenzaController extends SelectorComposer<Component> {
	...
	private int SEC_MUESTRA_INFLUENZA = 0;
	
	@Wire("[id$=muestraSerologiaInfluenza]")
	private Window wMuestraSerologiaInfluenza;
	...
	@Listen("onCreate=[id$=muestraSerologiaInfluenza]")
	public void onCreate_muestraSerologiaInfluenza() throws Exception{
	  init();
	...
	private void init() throws Exception {
		...
		ordenesExamenes = (OrdenesExamenes) wMuestraSerologiaInfluenza
				.getAttribute("ordenesExamenes");
		
		InfluenzaMuestra resultado = laboratorioService.obtenerInfluenzaMuestra(ordenesExamenes.getSecOrdenLaboratorio());
		if(resultado != null && resultado.getSecInfluenzaMuestra() != 0){
			txtCodigoMuestra.setText(resultado.getCodigoMuestra());
			SEC_MUESTRA_INFLUENZA = resultado.getSecInfluenzaMuestra();
 ...
 @Listen("onClick=[id$=btnCerrar]")
    public void btnCerrar_onClick() {
	 wMuestraSerologiaInfluenza.setAttribute("accionGuardado", false);
	 wMuestraSerologiaInfluenza.onClose();
    }
...
private void validarCamposRequerido() throws Exception{
	...
	if (this.cmbBioanalista.getSelectedIndex() <= 0) {
	 ...
	 guardaResultadosInfluenza();
	 
	}

private void guardaResultadosInfluenza() throws Exception{
	 
	InfluenzaMuestra examenMuestraInfluenza = new InfluenzaMuestra();
	...
	//SimpleDateFormat sdfHoraReporte = new SimpleDateFormat("hh:mm a");
		...
		if(SEC_MUESTRA_INFLUENZA != 0){
			examenMuestraInfluenza.setSecInfluenzaMuestra(SEC_MUESTRA_INFLUENZA);
		}
	 
	 examenMuestraInfluenza.setHoraReporte(horaReporte.getTime());
	 ...
	 examenMuestraInfluenza.setOrdenLaboratorio(ordenLaboratorio);
	 examenMuestraInfluenza.setCodigoMuestra(this.txtCodigoMuestra.getValue());
	 examenMuestraInfluenza.setUsuarioBioanalista(bioanalista);
	 examenMuestraInfluenza.setEstado('1');

	 InfoResultado result = laboratorioService.guardarMuestraInfluenza(examenMuestraInfluenza);
	 if(result.isOk() && result.getObjeto()!=null){
		 examenMuestraInfluenza = (InfluenzaMuestra)result.getObjeto();
		 
		 Mensajes.enviarMensaje(Mensajes.REGISTRO_GUARDADO, 
				 Mensajes.TipoMensaje.INFO);
		 wMuestraSerologiaInfluenza.setAttribute("accionGuardado", true);
		 wMuestraSerologiaInfluenza.onClose();
...
public void chkTomaMx_onCheck() {
	...
	wMuestraSerologiaInfluenza.setAttribute("accionTomaMx", true);
}
